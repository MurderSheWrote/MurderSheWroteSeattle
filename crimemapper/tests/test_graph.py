...
def test_random_colors_format_front():
    """Assert that front format is correctly formatted."""
    result = random_colors()
    assert result[:4] == 'rgb('


def test_random_colors_format_end():
    """Assert that end of string is formatted correctly."""
    result = random_colors()
    assert result[-1] == ')'


def test_random_colors_ints():
    """Assert that numbers are in the correct place."""
    result = random_colors()
    result = result[4:-1]
...
def test_offense_counter_tuple(test_list):
    """Assert that function returns a list of tuples with two items each."""
    result = offense_counter(test_list)
    assert len(result[0]) == 2


def test_offense_counter_contents_1(test_list):
    """Assert that first item in tuple is the offense description."""
    result = offense_counter(test_list)
    assert type(result[0][0]) == str


def test_offense_counter_contents_2(test_list):
    """Assert that second item in tuple is the the count of offenses."""
    result = offense_counter(test_list)
...
def test_color_applicator_tuple(test_list):
    """Assert that function returns a list of tuples of 3 items each."""
    result = offense_counter(test_list)
    result = color_applicator(result)
...
def test_color_applicator_contents_2(test_list):
    """Assert that second item in tuple is the offense count."""
    result = offense_counter(test_list)
    result = color_applicator(result)
    assert type(result[0][1]) == int
...
def test_color_applicator_contents_3(test_list):
    """Assert that third item in tuple is a string containing the rgb."""
    result = offense_counter(test_list)
    result = color_applicator(result)
...
def test_get_category():
    """Assert function returns None when item not in values."""
...
def test_get_category_3():
    """Assert function returns None when item is not string."""
    result = get_category([])
    assert result is None


def test_main_db_call(new_entry, clear_main_cache):
    """Assert function accesses database and returns a list."""
    result = main_db_call()
    assert type(result) == list


def test_main_db_call_empty_db(dbtransaction, clear_main_cache):
    """Assert if database is empty it will return a list."""
    with pytest.raises(ImportError):
        main_db_call()


def test_main_db_call_items(new_entry, clear_main_cache):
    """Assert function accesses database, returns list of sub-categories."""
    result = main_db_call()
...
def test_crime_dict_totals_contents(new_entry, clear_main_cache):
    """Assert function returns a list of tuples."""
    result = crime_dict_totals()
    assert type(result[0]) == tuple


def test_crime_dict_totals_inside_1(new_entry, clear_main_cache):
    """Assert function returns a list of tuples with formatted content."""
    result = crime_dict_totals()
    assert type(result[0][0]) == str
...
def test_crime_dict_totals_inside_4(new_entry, clear_main_cache):
    """Assert functions returns a list of tuples with formatted content."""
    result = crime_dict_totals()
    assert result[0][2][:4] == 'rgb('
...
def test_crime_dict_totals_key(new_entry, clear_main_cache):
    """Assert function converts from keys."""
...
def test_crime_category_breakdown_contents(new_entry, clear_main_cache):
    """Assert function returns a dictionary."""
